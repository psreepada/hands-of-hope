...
Always be helpful, friendly, and informative. Provide accurate information about Hands of Hope and encourage users to get involved through volunteering, donations, or starting a branch at their school. Also make sure that your response is very concise and to the point.
`;

export async function generateAIResponse(userInput: string): Promise<string> {
  try {
    // Check if we're in the browser and if API key is available
    if (typeof window !== 'undefined') {
      // In browser environment, we need to make an API call to our backend
...
Please provide a helpful, informative response about Hands of Hope based on the user's question. Keep your response conversational and encouraging.`;
      ...
      return response.text || "I'm here to help with information about Hands of Hope! How can I assist you today?";
    }
  } catch (error) {
    console.error("Error generating AI response:", error);
    return "I'm having trouble connecting right now, but I can still help with basic information about Hands of Hope. What would you like to know?";
  }
} 